...
void StreamProofESP::SurfaceHealth2(Entity* entity)
{
    if (!config->streamProofESP.surfacehealth)
        return;

    bbox_t bbox;
    int health = entity->health();
    int color_additive = (100 - health) * 2;
    color bar_color = color(50 + color_additive, 200 - color_additive, 50, 255);
    int bar_height = ((bbox.h - 2) * health) / 100;
    render::draw_filled_rect(bbox.x - 6, bbox.y, 4, bbox.h, color::black(245));
    render::draw_filled_rect(bbox.x - 5, bbox.y + 1, 2, bar_height, color(255,255,255));
    int text_height = bar_height - 3;
    int hp_offset = (health < 10) ? 12 : 16;
    if (health < 100)
        render::text(bbox.x - hp_offset, bbox.y + text_height, render::fonts::watermark_font, std::to_string(health), false, color::white(240));
}

void StreamProofESP::rendersurface() noexcept
{
    if (interfaces->engine->isInGame())
    {
        ...
        for (int i = 1; i <= interfaces->engine->getMaxClients(); i++) {
            ...
            StreamProofESP::SurfaceHealth2(entity);
...
void StreamProofESP::drawGUI(bool contentOnly) noexcept
{
    ...
    if (ImGui::BeginChild("##child", { 400.0f, 0.0f })) {
        ...
        if (currentCategory < 2) {
            ...
            ImGui::Checkbox("Health Bar", &config->streamProofESP.surfacehealth);
